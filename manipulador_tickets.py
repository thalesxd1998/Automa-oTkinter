...
class ManipuladorTickets:
    ...
    def obter_nome_cliente(self, ticket):
        """Prioriza o nome da organização quando disponível, ou usa o businessName do cliente."""
        try:
            client = ticket.get('clients', [{}])[0]  # Pega o primeiro cliente
            organization = client.get('organization')
            if organization:
                return organization.get('businessName', 'Nome Não Encontrado')
            else:
                return client.get('businessName', 'Nome Não Encontrado')  # Usa o businessName do cliente
        except Exception as e:
